...
def on_message(client, userdata, message):
    try:
        topic_parts = message.topic.split("/")  # Example: ["Room1", "sensor", "Temperature"]
        ...
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.%f")  # No timezone info
        ...
        if response.status_code == 201:
            print(f"Data stored successfully: {response.status_code}, Response: {response.text}")
        else:
            print(f"Failed to store data: {response.status_code}, Response: {response.text}")
