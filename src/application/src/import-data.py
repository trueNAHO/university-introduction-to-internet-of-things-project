...
def wait_for_mongodb():
    while True:
        try:
            ...
            print(
                f"[{time.strftime('%Y-%m-%dT%H:%M:%S')}] Waiting for MongoDB to start"
            )
            time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    mongo_import(
        "air_quality", "/data/Project_sensor_data/air_quality_sensor_data.json"
    )
    ...
    mongo_import(
        "light_intensity", "/data/Project_sensor_data/light_intensity_sensor_data.json"
    )
    mongo_import(
        "room_facilities", "/data/Project_sensor_data/room_facilities_data.json"
    )
    mongo_import("sound", "/data/Project_sensor_data/sound_sensor_data.json")
    mongo_import(
        "temperature", "/data/Project_sensor_data/temperature_sensor_data.json"
    )
